...
for (var district in global_data["City Wise"]["Sindh"]) {
  if (global_data["City Wise"]["Sindh"][district].district == "Karachi") {
    ...
    default_table_data[3].push(
      global_data["City Wise"]["Sindh"][district].total
    );
  }
}
...
// Prediction Graph
var today = new Date();
var prediction_graph_canvas = document
...
var prediction_graph = new Chart(
  prediction_graph_canvas,
  prediction_graph_config
);

var prediction_details_canvas = document
...
var prediction_accuracy_graph = new Chart(
...
function monthly_data(data) {
  ...
  var i = 0;
  while (
    month_data[i] === undefined &&
    i <= month_list[month_list.length - 1]
  ) {
    month_data.push([[], [], [], []]);
    i++;
  }
    ...
    for (var i = 0; i < new_data.length; i++) {
      ...
      for (var j = 0; j < data.length; j++) {
        if (j === 0) {
          ...
          if (i === 1)
            temp.push({ x: new Date(data[j].date), y: data[j].total_admitted });
          if (i === 2)
